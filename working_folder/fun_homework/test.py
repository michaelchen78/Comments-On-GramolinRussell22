...
data = {
    ...
    "G_E": [0.973, 0.974, 0.967, 0.933, 0.915, 0.94, 0.922, 0.92, 0.848, 0.881, 0.884, 0.867, 0.873, 0.849, 0.784, 0.79,
            0.725]
}
...
h_j = [-0.004, -0.008, -0.012,
       -0.016]  # defined based off of n and dx. the length must be n+1, and they must sum to dx.
...
boldXAlt = [0.28, 0.3, 0.32, 0.34, 0.36, 0.38, 0.40, 0.42, 0.44, 0.46, 0.48, 0.50, 0.52, 0.54, 0.56, 0.58, 0.60
            ]
'''create matrix bold big phi and calculate its inverse'''
boldBigPhi = np.empty(shape=(len(boldX), len(boldX)),
                          dtype=float)  # this is possible weak point, because data is not uniform, this is off?
# print(givenValuesOfX)
for idx, x in enumerate(boldXAlt):
    for jdx, y in enumerate(boldXAlt):
        if (y != x):
            # print(idx, " ",jdx)
...
for idx, h in enumerate(h_j):
     for jdx, x in enumerate(boldXAlt):
