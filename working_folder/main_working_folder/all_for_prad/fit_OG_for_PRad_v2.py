...
import fit_OG_return_val_indices
...
SPECTROMETERS = ["S"]  # List of spectrometers # CHANGE 1 HERE
...
def read_cs_data(data_file_name):  # [MOD 2A: pass specific file into read_cs_data]
    ...
    data["norms"] = [[int(i) for i in str(s).split(":")] for s in data["norms"]]  # CHANGE 2 HERE IDK WHY WEIRD?
    # Add filler index:
    ...
    for i in range(8):
        ...
        for j in range(33):
            if j % 8 == i:
                list1.append(j)
        list2 = []
        array = np.asarray(list1, dtype=int)
        list2.append(array)
        val_indices.append(list2)
    for i in range(10):
        list1 = []
        for j in range(38):
            k = j + 33
...
def main(order, reg_param):
    ...
    data = read_cs_data("PRadAlone.dat")[0]  # [MOD 2C: pass in data file, only return data]
