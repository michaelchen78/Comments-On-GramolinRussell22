...
def group_validation(data, order, norms, reg_param):
    ...
    for energy in BEAM_ENERGIES:
        for spectrometer in SPECTROMETERS:
            ...
            val_indices.append(list(np.where(data["E"] == energy)))
    running_train = 0
    running_test = 0
    ...
    return int("{:.0f}".format((running_train / 17)*10**-5)), \
        int("{:.0f}".format(running_test)*10**-5)  # [MOD 3A: return chi^2 values for alt_data_methods.py]
