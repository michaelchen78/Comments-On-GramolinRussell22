...
def get_table_ii(orders_included, optimal_reg_params, data, n_norm_params):
    ...
    for counter, order in enumerate(orders_included):
        ...
        best_params, chi2, _, L, cov = fit_OG_for_PRad.fit(data, data, order, best_reg_param)
        fit_params = best_params[n_norm_params:]
        fit_cov = cov[n_norm_params:, n_norm_params:]

        # Extract the radii:
        b2, b2_sigma = fit_OG_for_PRad.get_b2(fit_params, fit_cov)
        radius, radius_stat = fit_OG_for_PRad.get_radius(b2, b2_sigma)
...
def perform_group_val_with_best_normalizations(order, reg_param, n_norm_params, data):
    best_params, chi2, _, L, cov = fit_OG_for_PRad.fit(data, data, order, reg_param)
    normalizations = best_params[:n_norm_params]  # return best normalizations
    x,y = fit_OG_for_PRad.group_validation(data, order, normalizations, reg_param)
    print(x,y)
    return x, y