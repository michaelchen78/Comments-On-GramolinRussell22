...
# [EDIT 4]
from gramolin_repo.models import calc_cs, get_b2, get_init, get_radius

# [EDIT 2.A]
N_NORM_PARAMS = None  # Number of normalization parameters
BEAM_ENERGIES = None  # List of beam energies (in MeV)
SPECTROMETERS = None  # List of spectrometers


# [EDIT 1.A]
def read_cs_data(data_file_path):
    ...
    data = pd.read_csv(
        data_file_path, sep=" ", skiprows=1, usecols=cols.keys(), names=cols.values()
    )

    # [EDIT 2.B]
    # Creates a list of beam energies from the data file, assigns to BEAM_ENERGIES
    beam_energies_set = set()
    for beam_energy in data["E"]:
        beam_energies_set.add(beam_energy)
    global BEAM_ENERGIES
    BEAM_ENERGIES = list(beam_energies_set)
    # Creates a list of spectrometers from the data file, assigns to SPECTROMETERS
    spectrometers_set = set()
    for spectrometer in data["spec"]:
        spectrometers_set.add(spectrometer)
    global SPECTROMETERS
    SPECTROMETERS = list(spectrometers_set)
    # Finds the number of normalization parameters from the data file, assigns to N_NORM_PARAMS
    max_normalization_parameter_number = -1
    for normalization in data["norms"]:
        norm_string = str(normalization)
        there_is_a_colon = False
        colon_index = None
        for idx, character in enumerate(norm_string):
            if character == ":":
                there_is_a_colon = True
                colon_index = idx
        if there_is_a_colon:
            norm_string = norm_string[colon_index + 1:]
...
def group_validation(data, order, norms, reg_param):
    ...
    # [EDIT 3.C]
    return (running_train / 17), running_test
...
def main(order, reg_param):
    ...
    # [EDIT 1.B]
    data = read_cs_data("data/a1_cs.dat")
    ...
    # [EDIT 3.D]
    chi2_train, chi2_test = group_validation(data, order, normalizations, reg_param)
    print("chi^2_train = {:.0f}, chi^2_test = {:.0f}".format(chi2_train, chi2_test))
